 ...
 */
public class ClueminingDowload1 {
    public static void dowload(HttpServletResponse response, List<User_operation> list){
        ...
        try {
            ...
            String [] title={"公司名称","真实姓名","用户名","操作详情","操作时间","下载量"};
            ...
            for (int i = 0; i < list.size(); i++) {
             User_operation user=list.get(i);
                Object[] params1={user.getUser_company(),user.getRealname(),user.getUser_name(),user.getOperation_explain(),user.getOperation_time(),user.getDowloadcount()};
                // 创建Excel的sheet的一行
                int r=i+1;
